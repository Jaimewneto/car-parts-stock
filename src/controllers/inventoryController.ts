...
import { inventoryService, CreateInventoryInput, UpdateInventoryInput } from '../services/inventoryService';
...
export const inventoryController = {
  ...
  async getInventoryByWarehouse(c: Context) {
    try {
      ...
      const inventory = await inventoryService.getInventoryByWarehouse(warehouseId);
      return c.json({ success: true, data: inventory });
    } catch (error) {
      console.error('Error getting warehouse inventory:', error);
      return c.json({ success: false, error: 'Failed to get warehouse inventory' }, 500);
    }
  },
  ...
  async getInventoryByProduct(c: Context) {
    try {
      ...
      return c.json({ success: false, error: 'Failed to get product inventory' }, 500);
    }
  },
  ...
  async createInventory(c: Context) {
    try {
      ...
      const existingInventory = await inventoryService.getInventoryByProductAndWarehouse(
        data.productId,
        data.warehouseId
      );
      ...
      if (existingInventory) {
        return c.json({
          success: false,
          error: 'Inventory for this product in this warehouse already exists',
      ...
      return c.json({ success: false, error: 'Failed to create inventory' }, 500);
    }
  },
  ...
  async updateInventory(c: Context) {
    try {
      ...
      return c.json({ success: false, error: 'Failed to update inventory' }, 500);
    }
  },
  ...
  async deleteInventory(c: Context) {
    try {
      ...
      return c.json({ success: false, error: 'Failed to delete inventory' }, 500);
    }
  },
};
